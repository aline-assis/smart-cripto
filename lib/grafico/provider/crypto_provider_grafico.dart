...
class CryptoProviderGrafico extends ChangeNotifier {
  ...
  Future<List<Indicator>> fetchChartData(String symbol) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final json = jsonDecode(response.body);
        final timestampList =
            json['spark']['result'][0]['response'][0]['timestamp'] as List;
        final closeList = json['spark']['result'][0]['response'][0]
            ['indicators']['quote'][0]['close'] as List;
        final symbol =
            json['spark']['result'][0]['response'][0]['meta']['symbol'];
        final timezone =
            json['spark']['result'][0]['response'][0]['meta']['timezone'];
  ...
  Future<List<Quote>> fetchQuote(int count) async {
    final String url =
        'https://query2.finance.yahoo.com/v1/finance/screener/predefined/saved?formatted=true&scrIds=all_cryptocurrencies_us&start=0&count=$count';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        final json = jsonDecode(response.body);
